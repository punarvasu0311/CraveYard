  ...
  const payWithRazorpay = async () => {
    try {
      ...
      const options = {
        ...
        amount: amount * 100,
        currency: "INR",
        name: "CraveYard", //your business name
        description: "Food Order Payment",
        order_id: razorpayOrderId,

        handler: async (response: any) => { //this evokes when user clicks yes/no
          try {
      ...
      const razorpay = new (window as any).Razorpay(options);//this is possible because of script added in index.html
      razorpay.open(); //this opens page where yes/no appears in razorpay gateway
    } catch (error) {
      console.log(error);
