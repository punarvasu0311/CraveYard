...
const Cart = () => {
  ...
  const increaseQty = async (itemId: string) => {
    try {
      setLoadingItemId(itemId); //used in below html to show loading animation 
      await axios.put(
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
      ...
      <div className="space-y-4">
        {cart.map((cartItem: ICart) => {
          const item = cartItem.itemId as IMenuItem;//beacuse of populated by ref
          const isLoading = loadingItemId === item._id; //used to show loading animation from above logic
