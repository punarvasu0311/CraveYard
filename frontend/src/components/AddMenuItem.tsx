...
const AddMenuItem = ({ onItemAdded }: { onItemAdded: () => void }) => {
  ...
  const [image, setImage] = useState<File | null>(null); //uploaded images
  ...
  const handleSubmit = async () => {
    ...
    formData.append("file", image);//we recieve file from multer
    ...
    try {
      ...
      resetForm();
      onItemAdded();//this adds the new item to menu items
    } catch (error) {
      console.log(error);
